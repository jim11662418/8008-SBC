...
;;; This is a 1K Floating-Point Mandelbrot Grapher for the 8008
...
;;;
;;; edited to assemble with the AS Macro Assembler (http://john.ccac.rwth-aachen.de:8000/as/) 
;;; by Jim Loos 05/22/2023
;;;
;;; Jump to 0040H to start.
            ...
            ORG 40H
start:      JMP MANDELBROT

;;; Page one of SCELBAL has many constants and variables, of which the following are needed here:
OLDPG1:     db 000,000		    ; SIGN INDICATOR
...
T:          db 0,0,0,0
;
;  these are chosen to make nice picture for Mandelbrot
;   
...
CNT:        db 000              ;integer number of iterations of Mandelbrot loop
...
;      Y=2*X*Y+Y1
;      X=T
;      IF X*X+Y*Y>=4 THEN I=N+2
;     NEXT I
;     IF X*X+Y*Y>=4 THEN PRINT " ";
;     IF X*X+Y*Y<4 THEN PRINT "*";
;    PRINT
;  NEXT Y1
;
...
MANDELBROT:
           LHI 000              ;only LHI in this pgm; all data must reside on pg0 to fit in 1K        
           LLI (STARTY)&0FFH    ;  Y1=STARTY
           CAL FLOAD
           LLI (Y1)&0FFH 
           CAL FSTORE
           LLI (CNTY)&0FFH      ; CNTY = 21
           LMI Y_N ;025
Y1LOOP:    LAM                  ; print chr(64+CNTY)
           ADI 300
           CAL ECHO
...
FPCOMP:    LLI 124                ; However, if original sign was negative, must now restore
           LBI 003                ;The FPACC to negative by performing two's complement
;          JMP COMPLM             ;on FPACC. Return to calling rtn via COMPLM.
...
NONZAC:    LLI 136                ;If FPACC was non-zero then check to see if FPOP has
           ...
           RTZ                    ;Normalized FPOP must be zero. Answer is in FPACC!
                                  ...
                                  ;ponents before perforrning the addition of the mantissa.
